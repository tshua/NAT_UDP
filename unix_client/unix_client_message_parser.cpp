...
UnixClientMessageParser::UnixClientMessageParser(){
	sendBuf = recvBuf = NULL;
}

UnixClientMessageParser::UnixClientMessageParser(char* sendBuf, char* recvBuf, sockaddr_in *remoteaddr){
...
stUserInfo* UnixClientMessageParser::getUserByName(const char* userName)
{
	UserList_Iter it; 
	for( it = clientList.begin(); it != clientList.end(); it++  )
	{
...
stUserInfo* UnixClientMessageParser::getUserByIP(unsigned int IP)
{
	UserList_Iter it; 
	for( it = clientList.begin(); it != clientList.end(); it++  )
	{
...
int UnixClientMessageParser::login(string username)
{
...
int UnixClientMessageParser::logout(string username)
{
	remoteaddr->sin_family = AF_INET;
	remoteaddr->sin_port = htons(SERVER_PORT);
	remoteaddr->sin_addr.s_addr = inet_addr(SERVER_IP);
...
int UnixClientMessageParser::parse()
{
