...
UnixClient::UnixClient():sock(LOCAL_PORT, SERVER_PORT, SERVER_IP)
{
...
UnixClient::UnixClient(int local_port):sock(local_port, SERVER_PORT, SERVER_IP)
{
	messageParser.sendBuf = sock.sendBuf;
	messageParser.recvBuf = sock.recvBuf;
	messageParser.remoteaddr = &(sock.get_remoteaddr());	
...
void UnixClient::login()
{
	sock.send(messageParser.login(myName));
}

void UnixClient::parseMessage()
{
	while(1)
	{
...
bool UnixClient::sendMessage(const char* username, const char* message)
{
	recvACK = false;
...
int UnixClient::parseCommoned(string commond)
{
